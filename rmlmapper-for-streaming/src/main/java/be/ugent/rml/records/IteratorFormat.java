...
public abstract class IteratorFormat<DocumentClass> implements ReferenceFormulationRecordFactory {
    ...
    public List<Record> getRecords(Access access, Term logicalSource, QuadStore rmlStore, String record_str) throws IOException, SQLException, ClassNotFoundException {
        ...
        if (! documentMap.containsKey(access)) {
            ...
            InputStream inputStream = new ByteArrayInputStream(record_str.getBytes());
            DocumentClass d = getDocumentFromStream(inputStream, access.getContentType());
            documentMap.put(access, d);
        }
